...
class DarkroomTest < Minitest::Test
  ...
  context(Darkroom, '#asset') do
    ...
    test('only returns asset if path excludes prefix when a prefix is configured and asset is pristine') do
  ...
  context(Darkroom, '#asset_path') do
  ...
  context(Darkroom, '#asset_integrity') do
    test('returns subresource integrity string according to algorithm argument') do
      ...
      assert_equal('sha512-VAhb8yjzGIyuPN8kosvMhu7ix55T8eLHdOqrYNcXwA6rPUlt1/420xdSzl2SNHOp93piKyjcNkQwh2L'\
        'w8imrQA==', darkroom.asset_integrity('/app.js', :sha512))
    end
